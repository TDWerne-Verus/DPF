...
for file in csv_files:
    ...
    if (file.find(Ch1) > -1):
        ...
        pear_att = att(pear, att_fact_pear)  # voltage, undoes hardware attenuation
        ...
        int_rogo = df['CH2'];  # K*dI/dt
        int_rogo_att = att(int_rogo, att_fact_rogo)     # undoes hardware attenuation
        rogo = derivate(int_rogo_att, 1, 1/Fs)  # K*I
        # int_rogo = MAF(int_rogo, order)
        for x in range(len(int_rogo)):
            rogo_curr.append(int_rogo_att[x]/4)  # divided by 4 loops
            # slow we can make faster by not dynamically allocating
        rogo_curr = butter_lowpass(rogo_curr, Fs, 10000, order=5)
        ...
        for t in range(refLength+1, len(CFARThreshold)-refLength):
        ...
        th = np.abs(rogo_curr) > (0.80 * max(np.abs(rogo_curr)))
        th = np.abs(pear_curr) > 2000


        occurrences_of_true = np.where(th == True)

        if (isinstance(occurrences_of_true, int)):
            start = occurrences_of_true
        else:
            start = occurrences_of_true[0][0]
            if (start == 0):
                start = occurrences_of_true[0][1]
        ...
        # end = end[0]
        '''
        th = [y > 0.8*max(rogo_curr) for i, y in enumerate(rogo_curr)]
        for i in range(0,len(rogo)):
            if(th[i] == True):
                start = i
                break;
        '''
        x_peaks = time[start:end]

        y_peaks = rogo_curr[start:end]
        ...
        sensitivities = sensitivities[0:n]
        sensitivity = np.mean(sensitivities)
        #n_peaks = np.asarray(n_peaks)
        sensitivities = np.ndarray.tolist(sensitivities)
        plt.figure('Sensativity Over All Data')
        plt.grid(True)
        #plt.xlim(time[start], time[end])
        plt.scatter(n_peaks, sensitivities)
        plt.xlabel('Time [s]')
