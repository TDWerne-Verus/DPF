...
for file in csv_files:
    ...
    Fs = 1000e6
    time_fix = np.linspace(time[0], time[time.size - 1], num=int(1 + Fs * (time[time.size - 1] - time[0])))
    ...
    plt.plot(time, pear_curr, time, rogo_curr2)
    # , time, pass_curr)
    plt.xlabel('Time(us)')
    plt.ylabel('Current (A))')
    plt.legend(['Pearson = %0.2f' % pear_peak, 'Mathematical Rogowski = %0.2f' % int_peak2])
    ...
    plt.xlim(0, 20)
    time = [x * 1e6 for x in time]
    plt.plot(time, rogo_curr2, time, rogo_curr3, time, rogo_curr4)
    plt.xlabel('Time(us)')
    plt.ylabel('Current (A))')
    '''
...
'''
pear_diff = (max(pear_array) - min(pear_array)) * 100 / (sum(pear_array) / len(pear_array))
...
sense_diff3 = ((max(sense_array3) - min(sense_array3)) * 100) / (sum(sense_array3) / len(sense_array3))
